...
![Reponary logo](frontend/public/logo-horizontal.svg)

**Local projects. Deeper understanding.**

**Early preview · 0.1.0 · local-first**

Reponary is a local-first developer tool that brings code intelligence and engineering knowledge together in one workspace. Code Intelligence shows **what your code is now**; Engineering Knowledge preserves **why it became that way** through Markdown notes, decisions, incidents, and project context. Graphify integration provides the current code graph.
...
> Reponary is not affiliated with Graphify or Obsidian. Both are independent, optional products owned by their respective creators. Neither is bundled here.
...
- Prepare a prompt for an external coding agent to propose knowledge for human review. Reponary itself sends nothing to an AI provider.
...
- Graphify CLI is optional. If installed, it runs on the host in local code-only mode for new graphs. Non-code semantic extraction is not invoked by Reponary.
...
```sh
git clone https://github.com/joaofernandesuk/reponary.git
cd reponary
./install.sh
```
...
The installer generates a local access key on first run and prints it at the end. This is Reponary's own sign-in key, not a GitHub credential. Open the printed `http://127.0.0.1:8765` URL and enter the key. It remains in `~/.engineering-knowledge-hub/agent/web.token` (or under your custom `HUB_DATA_DIR`) if you need it again. On macOS, `pbcopy < ~/.engineering-knowledge-hub/agent/web.token` copies the default key without displaying it. Choose an existing Markdown directory, or create a directory first and select it. Then click **+ Add Project** and enter the absolute path to a source folder.

If port 8765 is occupied, set `HUB_PORT=8766` before running the installer. Set `HUB_DATA_DIR` to change the private runtime location or `HUB_INSTALL_DIR` to change the installed application copy. To update an existing installation, run `git pull --ff-only` and `./install.sh` again from the checkout. The installer reuses that installation's private runtime and vault configuration. It does not import or modify another installation.

For v0.1.x compatibility, the default private runtime remains `~/.engineering-knowledge-hub/`, the installed application copy remains `~/.local/share/engineering-knowledge-hub/`, and existing `HUB_*` settings and Docker identifiers remain valid. The Reponary name does **not** move or delete runtime state; a path migration would be a separate, explicit operation.
...
1. Open the local Reponary URL and sign in with the local access key.
...
4. Follow actual progress. The browser reconnects if Reponary restarts to add a read-only graph mount.
...
The small native agent validates paths and performs the allow-listed host operations. Docker Desktop on macOS cannot use the tested bind-mounted Unix socket, so the agent polls an authenticated localhost endpoint on Reponary. Native runs can use the Unix socket. The container never receives the Docker socket, a home-directory mount, or repository write access.
...
Back up the private runtime directory and your Markdown folder separately. They are ordinary files; stop Reponary before copying the runtime for a consistent registry snapshot. Keep the copy access-controlled because it contains local access keys. Restore them to their original paths, then run `./install.sh` from the source checkout. Graphify outputs remain in their source folders and are not part of a Reponary backup. Reponary is not a repository backup system.

Run `./uninstall.sh` to stop Reponary's container and host agent. The runtime, repositories, graph outputs, and Markdown vault remain. Remove the runtime manually only after confirming a backup; the uninstall script deliberately does not delete user data.
